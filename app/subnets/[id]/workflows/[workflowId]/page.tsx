...
import { Pause, RotateCcw, Activity, BarChart3, Circle } from "lucide-react"
import Link from "next/link"

// Mock data for workers
const workersData = [
  { id: "worker-node-01", status: "active", currentActivity: "act-003", tasksCompleted: 45 },
  { id: "worker-node-05", status: "active", currentActivity: "act-004", tasksCompleted: 38 },
  { id: "worker-node-12", status: "idle", currentActivity: "-", tasksCompleted: 52 },
]

// Mock data for activities
const activitiesData = [
  { id: "act-001", name: "Validate Input", status: "succeeded", duration: "1.2s", worker: "worker-node-01" },
  { id: "act-002", name: "Fetch Data", status: "succeeded", duration: "2.8s", worker: "worker-node-05" },
  { id: "act-003", name: "Process Data", status: "running", duration: "5.3s", worker: "worker-node-01" },
  { id: "act-004", name: "Transform Results", status: "pending", duration: "-", worker: "-" },
  { id: "act-005", name: "Store Output", status: "pending", duration: "-", worker: "-" },
]
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="p-6 space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <Card className="p-4">
            <div className="text-sm text-muted-foreground mb-1">Current Activity</div>
            <div className="text-sm">Process Data (act-003)</div>
          </Card>
          <Card className="p-4">
            <div className="text-sm text-muted-foreground mb-1">Activities Completed</div>
        ...
        <Card className="border border-border rounded-lg overflow-hidden">
          <Accordion type="multiple" defaultValue={["event-history"]} className="w-full">
            ...
            <AccordionItem value="activities" className="border-b border-border px-6">
                    ...
                    <tbody>
                      {activitiesData.map((activity) => (
                        <tr key={activity.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                          <td className="py-3 px-4 font-mono text-xs">{activity.id}</td>
                          <td className="py-3 px-4 text-sm">{activity.name}</td>
                          <td className="py-3 px-4">
                            <WorkflowStatusBadge status={activity.status as any} />
                          </td>
                          <td className="py-3 px-4 font-mono text-sm">{activity.duration}</td>
                          <td className="py-3 px-4 font-mono text-xs text-muted-foreground">{activity.worker}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <AccordionItem value="event-history" className="border-b border-border px-6">
              <AccordionTrigger className="hover:no-underline py-4">
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {workersData.map((worker) => (
              <Card key={worker.id} className="p-4 hover:bg-[#1E1F22FF] transition-colors cursor-pointer">
                <div className="flex items-start justify-between">
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <h4 className="text-sm font-mono font-medium">{worker.id}</h4>
                    </div>
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">{worker.currentActivity}</span>
                      </div>
                      <div className="flex justify-between">
